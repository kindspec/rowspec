...
## What this one guards

`render` must put the escape back. Without it the line gains a delimiter and one
cell silently becomes two — the defect the `render-does-not-escape-pipes` mutant
injects. Byte-exact round-trip is the cheapest assertion that catches it.

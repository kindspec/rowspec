...
This is the positive half, and the one that fails if the refusal over-correction
is taken: the column named `1e3` exists, so the formula resolves to it and the
answer is 10.0. A reader that refuses the *name* cannot represent this table at
all.

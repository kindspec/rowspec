...
Here no column named `1_0` exists. The token is still one `ident`, so the answer
is §8's ordinary broken reference — `#REF!(1_0)`, naming the thing that was not
found. A reader that lexed `1_0` as the digit-separated number 10 would return
70.0 instead, which is a plausible number for a formula the author did not
write.

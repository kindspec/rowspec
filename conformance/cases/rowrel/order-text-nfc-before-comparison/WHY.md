...
This fixture spells the third name NFD (`A` + U+0301). §4.1.8 requires the
comparison to run over the **NFC-normalised** value, so it must sort exactly
where the precomposed `Ápple` sorts -- last. Compared raw, `Ápple` sorts
immediately after `Apple` (U+0301 is 769, `p` is 112), giving lo = 10.0 and
hi = 110.0 instead.
